...
export class IncidentsService {
  ...
  listIncidents(query = ''): Promise<Incident[]> {
    return this.api.get<Incident[]>(`/api/incidents${query}`);
  }

  getIncident(id: string | number): Promise<Incident> {
    return this.api.get<Incident>(`/api/incidents/${id}`);
  }

  listAlerts(): Promise<Alert[]> {
    return this.api.get<Alert[]>('/api/incidents/alerts');
  }
}
